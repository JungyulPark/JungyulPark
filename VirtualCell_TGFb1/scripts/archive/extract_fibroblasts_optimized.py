...
def main():
    ...
    print("=== Phase 1: Reading Metadata (Default Caching) ===")
    start_time = time.time()
    
    # Open with default fsspec caching for fast metadata scanning
    with fsspec.open(url, 'rb').open() as f:
        with h5py.File(f, 'r') as h:
            print("Successfully opened remote file for metadata.")
    ...
    # Optimization: Group nearby range requests
    ranges = []
    ...
    print("=== Phase 2: Extracting Expression Matrix (Caching Disabled) ===")
    ...
    # Open with cache disabled and seekable mode for precise range requests
    with fsspec.open(url, 'rb', cache_type='none', block_size=1024*1024).open() as f:
        with h5py.File(f, 'r') as h:
            ...
            for i, (m_start, m_end, cells_info) in enumerate(merged_ranges):
                if i % 10 == 0:
                    print(f"  Fetching chunk {i}/{len(merged_ranges)} ({m_end - m_start} elements)...")
                
                # Fetch bulk range in single HTTP request
